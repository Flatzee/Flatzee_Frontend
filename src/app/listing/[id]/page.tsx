...
export default function ListingPage({ params }: { params: { id: string } }) {
  ...
  if (!listing) {
    return (
      <div className="p-6">
        <button onClick={() => router.push("/feed")} className="mb-4 inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-sm hover:bg-neutral-50"><ArrowLeft className="h-4 w-4" /> Back</button>
  ...
  return (
    <div className="min-h-screen bg-neutral-50">
      <div className="sticky top-0 z-30 flex h-14 items-center gap-2 border-b bg-white/80 px-3 backdrop-blur">
        <button aria-label="Back to feed" onClick={() => router.push("/feed")} className="rounded-full p-2 hover:bg-neutral-100 focus:outline-none focus-visible:ring"><ArrowLeft className="h-5 w-5" /></button>
      ...
      <div className="mx-auto grid max-w-6xl gap-6 p-4 md:grid-cols-[1.15fr_.85fr] md:py-6">
        <div className="space-y-6">
          <div>
            ...
            <div className="mt-1 flex flex-wrap items-center gap-2 text-sm text-neutral-700">
              <span className="inline-flex items-center"><Star className="mr-1 h-4 w-4" /> {listing.rating} ({listing.reviews})</span>
              {listing.verified && (
                <span className="inline-flex items-center"><BadgeCheck className="mr-1 h-4 w-4" /> Verified</span>
              )}
          ...
          <div>
            ...
            <p className="text-sm text-neutral-700">A comfortable, light-filled apartment perfect for couples or solo travelers. Close to cafes and public transport. Enjoy a quiet night with city views and thoughtful amenities throughout your stay.</p>
          </div>

          <div>
            <h2 className="mb-2 text-base font-semibold">Amenities</h2>
            <div className="grid grid-cols-2 gap-2 text-sm md:grid-cols-3">
              {["Wi-Fi","Kitchen","Balcony","Parking","Washer","24/7 Support","Air conditioning","Hair dryer","Essentials","TV","Workspace","Smoke alarm"].map((a) => (
                <div key={a} className="rounded-lg border bg-white p-3">{a}</div>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-3 rounded-2xl border bg-white p-4">
              ...
              <div className="text-sm">
                ...
                <div className="text-neutral-600">{listing.hostYears} years hosting {listing.verified && "- Verified"}</div>
              </div>
            </div>
          </div>
        ...
        <aside className="h-max space-y-3 rounded-2xl border bg-white p-4">
          <div className="text-lg font-semibold">NPR {listing.pricePerNight.toLocaleString()} <span className="text-sm font-normal text-neutral-500">/ night</span></div>
          <DateRangePicker value={range} onChange={setRange} />
          <GuestPicker defaultValue={{ adults: 2 }} onComplete={(total) => setGuests(total)} />
          <button aria-label="Request to book" className="mt-2 w-full rounded-lg bg-black px-4 py-2 text-white hover:opacity-90 focus:outline-none focus-visible:ring">
            Request to book
          </button>
